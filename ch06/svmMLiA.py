...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
  ...
  while (iter < maxIter):
    ...
    for i in range(m):
      ...
      # error Ei based on the prediction and the real class of this instance
...
'''
kernel transformation funciton
'''
def kernelTrans(X, A, kTup):
  m,n = shape(X)
  K = mat(zeros((m,1)))
  if kTup[0] == 'lin': K = X * A.T  # linear case: dot product
  elif kTup[0] == 'rbf':
    for j in range(m):              # radial bias function: Gaussian function
      deltaRow = X[j,:] - A
      K[j] = deltaRow*deltaRow.T
    # 1: elemten-wise division
    K = exp(K / (-1*kTup[1]**2))
